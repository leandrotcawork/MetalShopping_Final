...
import { makeAnalyticsHomeV2Dto, type AnalyticsHomeV2Dto } from "@metalshopping/feature-analytics";
...
export function AnalyticsPage() {
  ...
  const loadAnalytics = useCallback(async () => {
    ...
    setPageError("");
    try {
      const envelope = await api.home.workspace(undefined, { includeOperational: false });
      ...
      setPageError(apiErr?.message || (err instanceof Error ? err.message : String(err)));
    }
  ...
  useEffect(() => {
    ...
    async function loadTaxonomyRanking() {
      ...
      try {
        ...
        if (disposed) return;
        const apiErr = err instanceof ApiClientError ? err : null;
        setTaxonomyRankingError(apiErr?.message || (err instanceof Error ? err.message : String(err)));
        setTaxonomyRankingRows([]);
      }
    }
  ...
  useEffect(() => {
    ...
    async function loadTaxonomy() {
      ...
      try {
        ...
        if (disposed) return;
        const apiErr = err instanceof ApiClientError ? err : null;
        setTaxonomyError(apiErr?.message || (err instanceof Error ? err.message : String(err)));
        setTaxonomyLevels([]);
        setTaxonomyRoots([]);
      } finally {
  ...
  return (
    <section className="ah-page">
      ...
      {activeTab === "home" ? <AnalyticsHomePage /> : null}
      ...
      <div className={`ah-backdrop${drawer ? " open" : ""}`} onClick={() => setDrawer(null)} />
      <aside className={`ah-drawer${drawer ? " open" : ""}`}>
        <header><h3>{selectedAction?.title || selectedAlert?.title || (drawer?.type === "sku" ? `SKU ${drawer.sku}` : "Spotlight")}</h3><button type="button" onClick={() => setDrawer(null)}>x</button></header>
        <div>
          {selectedAction ? (
            <>
              <h4>Why</h4>
              ...
              <h4>Next steps</h4>
              <div className="ah-drawer-actions"><button type="button" onClick={() => navigate("/analytics/actions")}>Abrir no Kanban</button><button type="button">Exportar</button></div>
              <h4>Ver SKUs</h4>
              <div className="ah-drawer-skus">{selectedAction.topSkus.map((sku) => <button key={sku} type="button" onClick={() => setDrawer({ type: "sku", sku })}>{sku}</button>)}</div>
            </>
          ) : null}
          {selectedAlert ? (
            <>
              ...
              <div className="ah-drawer-skus">{selectedAlert.topSkus.map((sku) => <button key={sku} type="button" onClick={() => setDrawer({ type: "sku", sku })}>{sku}</button>)}</div>
            </>
          ) : null}
          {drawer?.type === "sku" ? <p>SKU Drawer Lite para {drawer.sku}. Workspace de produto ainda em construcao.</p> : null}
          {!drawer ? <p>Clique em cards para abrir detalhes.</p> : null}
        </div>
      </aside>
    </section>
  );
