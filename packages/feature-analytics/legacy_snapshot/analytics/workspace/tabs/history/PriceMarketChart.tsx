...
export function PriceMarketChart({ points, supplierLinks = {} }: PriceMarketChartProps) {
  ...
  const chartLabels = filtered.map((point) => point.x.slice(5));
  const ourData = filtered.map((point) => point.our);

  const competitorNames = Array.from(
    new Set(filtered.flatMap((point) => Object.keys(point.competitors || {}))),
  );
  ...
  const competitorSeriesByName = useMemo<Record<string, Array<number | null>>>(() => {
    ...
    competitorNames.forEach((name) => {
      const rawSeries = filtered.map((point) => point.competitors[name] ?? null);
