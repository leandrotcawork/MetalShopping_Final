# Canonical Lessons — always active

Copy this entire file into tasks/lessons.md at project start.
...
Rule: set_config before every write transaction. tenant_id from event, never hardcoded.
Layer: Python worker
...
Wrong:  Creating StatusBadge when StatusPill exists
...
Correct: Write → implement → run acceptance test → commit "docs(adr): ADR-XXXX — verified"
Rule: ADR Status: Accepted requires passing acceptance test and git commit.
Layer: Process
